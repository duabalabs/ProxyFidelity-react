...
import { SingleElementForm, Text } from "@/dashboard/components";
import type {
  BusinessType,
  PeopleSize,
  Industry,
} from "@/graphql/schema.types";
import type { PeopleInfoQuery } from "@/graphql/types";
import { currencyNumber } from "@/utilities";

import { COMPANY_INFO_QUERY } from "./queries";

type People = GetFields<PeopleInfoQuery>;
...
export const PeopleInfoForm = () => {
  ...
  const { query: queryResult } = useShow<People>({
    meta: {
  ...
  const { totalRevenue, industry, peopleSize, businessType, country, website } =
    data || {};

  const getActiveForm = (args: { formName: keyof People }) => {
    const { formName } = args;
  ...
  return (
    ...
    >
      <SingleElementForm
        ...
        state={getActiveForm({ formName: "peopleSize" })}
        itemProps={{
          name: "peopleSize",
          label: "People size",
        }}
        view={<Text>{peopleSize}</Text>}
        onClick={() => setActiveForm("peopleSize")}
        onUpdate={() => setActiveForm(undefined)}
        onCancel={() => setActiveForm(undefined)}
      >
        <Select
          autoFocus
          defaultValue={peopleSize}
          options={peopleSizeOptions}
          style={{
            width: "100%",
          }}
        />
