...
import type {
  PeopleTitleFormMutation,
  PeopleTitleFormMutationVariables,
} from "@/graphql/types";
...
export const PeopleTitleForm = () => {
  const {
    formProps,
    query: queryResult,
    onFinish,
  } = useForm<
    GetFields<PeopleTitleFormMutation>,
    HttpError,
    GetVariables<PeopleTitleFormMutationVariables>
  >({
    redirect: false,
    meta: {
  ...
  const people = queryResult?.data?.data;
    ...
    <Form {...formProps}>
      <Space size={16}>
        <CustomAvatar
          ...
          name={getNameInitials(people?.name || "")}
          style={{
            width: 96,
            height: 96,
            fontSize: 48,
